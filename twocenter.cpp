...
// Calculate the distance between i and j with BFS
int distance(vector<vector<int> > *paths, int i, int j){
	...
	bool flag = true;					// The flag to check if we found j or not.
	...
	while (!BFS.empty() && flag) {
		...
		Then, for every neighbors(element), we check if it has been seen and if we found j.
		If this is the case, then we update BFS, seen and depth with the values of neighbor.
		If we found j, we change flag and the loop while ends. */
		head = BFS.front();
		BFS.pop();
		
		for (int k = 0; k < (*paths)[head].size(); k++){		// Check the neighbors of head.
			neighbor = (*paths)[head][k];
			if ((neighbor != j) && (!seen[neighbor])){			// Update
				seen[neighbor] = true;
				BFS.push(neighbor);
				depth[neighbor] = depth[head] + 1;
			}
			else if (neighbor == j){								// We found j
				depth[neighbor] = depth[head] + 1;
				flag = false;
				break;
			}
...
// Calculate the distance between every vertex with Floyd-Warshall.
vector<vector<int> > all_distance_DP(vector<vector<int> > *paths){
	int N = (*paths).size();			// The size of the vertex.
	vector<vector<int> > distance_between_vertex (N, vector<int> (N, INT_MAX));		// The Floyd-Warshall matrix.
	int neighbor;						// A variable to make the code more readable.
	int distance_temp;					// Cache to calculate the distance.

	for (int i = 0; i < N; i++){		// Initialization with distance(i, i) = 0
		distance_between_vertex[i][i] = 0;
	}
	
	for (int i = 0; i < N; i++){		// Initialization with distance(i, j) = 1 if i is a neighbor of j and vice versa.
		for (int j = 0; j < (*paths)[i].size(); j++){
			neighbor = (*paths)[i][j];
			distance_between_vertex[i][neighbor] = 1;
			distance_between_vertex[neighbor][i] = 1;
			}
		}
	...
	for (int k = 0; k < N; k++){
		for (int i = 0; i < N; i++){
			for (int j = i + 1; j < N; j++){
				if ((distance_between_vertex[i][k] != INT_MAX) && (distance_between_vertex[k][j] != INT_MAX)){
					distance_temp = distance_between_vertex[i][k] + distance_between_vertex[k][j];

					if (distance_between_vertex[i][j] > distance_temp){
						distance_between_vertex[i][j] = distance_temp;
						distance_between_vertex[j][i] = distance_temp;
					}
				}
	...
	return distance_between_vertex;
}


// Calculate the score with vertex i and j.
long long score(int N, vector<vector<int> > *paths, vector<int> *weights, int i, int j){
	long long s = 0;
	vector<int> distance_i = all_distance(paths, i);			// The distance between i and all the vertex.
	vector<int> distance_j = all_distance(paths, j);			// The distance between j and all the vertex.
	for (int k = 0; k < N; k++){								// Sum them up with weights.
		s += (*weights)[k] * min(distance_i[k], distance_j[k]);
	}
	return s;
}

// Calculate the score with vertex i and j while we have already every distance between vertex. (+DP for the score)
long long score_with_distances(int N, map<pair<int, int>, long long> *score_DP, vector<vector<int> > *distance_between_vertex, vector<int> *weights, int i, int j){
	if (i == j) return LONG_MAX;
	int temp_min = min(i, j);
	int temp_max = max(i, j);
	i, j = temp_min, temp_max;
...
long long score_with_distances_greedy(int N, map<pair<int, int>, long long> *score_DP, vector<vector<int> > *distance_between_vertex, vector<vector<int> > *paths, vector<int> *weights, int i, int j){
	i, j = min(i, j), max(i, j);
	...
	if ((best_indice_i == i) && (best_indice_j == j)) return best_score;
	if ((best_indice_j == i) && (best_indice_i == j)) return best_score;
...
int main(){
	...
	// method Floyd-Warshall for every distance between vertex. (slower)
		...
		for (int a = 0; a < N; a++){
			for (int b = a + 1; b < N; b++){
				score_temp = score_with_distances(N, &score_DP, &distance_between_vertex, &weights, a, b);
